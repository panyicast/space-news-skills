...
## GitHub Deep Research 的使用时机

以下情况适合调用 `github-deep-research`：
- 某个 GitHub 仓库已经在初筛中被判定为高价值
- 需要分析仓库活跃度、提交节奏、Issue / PR 信号
- 需要补充竞品关系、版本演化或维护状态判断
- 需要把“看起来重要的开源项目”进一步转化成可写入简报的结构化判断

以下情况不建议调用：
- 只是为了确认仓库是否存在
- 只是需要 README 级别的基础信息
- 当前主题与 GitHub / 开源项目关系很弱
- 已有官方源、论文源或项目页足以支撑写作，且没有必要进一步挖 commit / issue / PR

使用后的并入原则：
- 它的输出用于补充判断，不直接等同于最终来源
- 最终写入简报时，仍应尽量链接到仓库、release、docs、论文页、项目主页或其他原始页面
- 若其分析结论与官方或项目原始页面不一致，优先回查原始证据
...
- 默认优先使用当前可用的 Tavily 作为发现层；Brave 可作为补充搜索源，但不是必需依赖。
- `github-deep-research` 可作为 GitHub / 开源项目深挖工具，用于补充仓库、提交、Issue、PR、版本节奏和竞争分析。
- 搜索接口如 Tavily、Brave，及研究工具如 `github-deep-research`，都不能作为唯一来源依据。
